...
def load_data(uploaded_file, file_name):
    dataframe_version = pd.read_csv(
        uploaded_file, encoding="utf-8", delimiter=csv_delimiter
    )
...
with st.sidebar:
    uploaded_file = st.file_uploader(
        "Choose a CSV file from your computer", accept_multiple_files=False
    )
    ...
    if plot_description:
        response_dataset_description = openai.ChatCompletion.create(
            ...
            messages=[
                {
                    "role": "user",
                    "content": "I need you to read the following data:"
                    + st_version[0:200]
                    + "\n"
        ...
        created_code = extract_string_between_triple_backticks(
            response_dataset_summary.choices[0].message.content
        )
